...
String playerId, dirParam, displayName, imageURLExpansionPlus, imageURLExpansionMinus,
	selfStarImagePath, soundNormalFailureBuzz;
...
soundNormalFailureBuzz = Constants.soundDirectoryURLPath+"/"
		+Configuration.getConfigProperties().getProperty(Constants.config_client_failureBuzzFileName);
...
<script type="text/javascript" src="include/popupsPFN.jsp"></script>
<script type="text/javascript" src="include/playSectionPFN.jsp"></script>
<% if (isSelfDirector) { %>
<script type="text/javascript" src="include/scoreEditorPFN.jsp"></script>
...
let soundOption = SoundType.silly;
...
function handleVolumeSlider(event) {
	...
	audioElement.volume = (parseFloat(element.value) / 100.0);
	
	console.log("handleVolumeSlider():: volume set to " + audioElement.volume);
}
...
function handleSoundRadioButton(event)
{
	...
	if (src.checked) {
		if (srcId=="radioNoSound") {
			soundOption = SoundType.muted;
		} else if (srcId=="radioNormal") {
			soundOption = SoundType.normal;
		} else if (srcId=="radioSilly") {
			soundOption = SoundType.silly;
		} else {
			soundOption = SoundType.muted;
		}
	
		audioElement=document.getElementById("audioTag");
		if (soundOption == SoundType.muted) {
...
//********** Regular functions *****************
/**
 * Sounds should always be fired by events.
 *
 * @param frontEndEvent A value from the backend.
 * @param shouldOnlySendToCurrentPlayer If true, send the sound to the current
 * player only.  Otherwise, send it to everyone.
 */
function handlePotentialAudio(frontEndEvent, shouldOnlySendToCurrentPlayer=true) {
	let audioElement, whichFileNumber, whichFileName, whichFileIndex,
		shouldPlayAudio;
	
	
	if (soundOption != SoundType.muted) {
		
		shouldPlayAudio = shouldOnlySendToCurrentPlayer ? isSelfCurrentPlayer() : true;

		if (shouldPlayAudio) {
			soundFileURL = null;
			audioElement=document.getElementById("audioTag");
			if (frontEndEvent.eventType == "<%= FEEventType.failedWithCard %>") {
				if (soundOption == SoundType.silly) {
					whichFileIndex = Math.trunc(Math.random() * globalAllSoundFileNames.length);
					soundFileURL = "<%= Constants.soundDirectoryURLPath %>/" + globalAllSoundFileNames[whichFileIndex];
				} else {
					soundFileURL = "<%= soundNormalFailureBuzz %>";
				}
			}
			
			if (soundFileURL) {
				audioElement.src = soundFileURL;
				audioElement.play();
			}
		}
	}
...
function isGameNotRunning(gameState)
{
	return gameState == "<%= GameState.ended %>"
		|| gameState == "<%= GameState.gameAborted %>"
		|| gameState == "<%= GameState.notStarted %>";
}


function isGameRunning(gameState)
{
	return !isGameNotRunning(gameState);
}

function isSelfCurrentPlayer()
{
	let retVal;
	
	retVal = globalVars.currentPlayerId == "<%= playerId %>";
...
function fillGlobalVariables(statusResponseObj)
{
	...
	if (statusResponseObj.teamInfo) {
		globalVars.currentPlayerId = statusResponse.teamInfo.currentPlayerId;
		if (statusResponseObj.teamInfo.teamList)
		{
...
function fillSuccessOrFailFadingImageAndPlaySound(statusResponse)
{
	let storedImage, span, newImage, isFailure, isSuccess;
	
	storedImage=null;
	isSuccess=isFailure=false;
	
	if (statusResponse.frontEndEvent) {
		if (statusResponse.frontEndEvent.eventType == "<%= FEEventType.successfullyScoredCard %>") {
			storedImage = document.getElementById("storedImage_success1");
			isSuccess = true;
		} else if (statusResponse.frontEndEvent.eventType == "<%= FEEventType.successfullyScoredCard3 %>") {
			storedImage = document.getElementById("storedImage_success3");
			isSuccess = true;
		} else if (statusResponse.frontEndEvent.eventType == "<%= FEEventType.failedWithCard %>") {
			isFailure = true;
			storedImage = document.getElementById("storedImage_failure");
		}
	
		
// 		} else if (statusResponseObj.frontEndEvent.isNewGame) {
//     	    resetModIds();
//     	    resetGlobalIdsToDefaults();
//     	    performCloseVictoryPanel();
//      }
// else if (statusResponseObj.frontEndEvent.eventType == "yourTurnEnded") {
// 			storedImage = document.getElementById("storedImage_yourturnhasended");
// 		} else if (statusResponseObj.frontEndEvent.eventType == "itsYourTurn") {
// 			storedImage = document.getElementById("storedImage_itsyourturn");
// 		} else if (statusResponseObj.frontEndEvent.eventType == "gameEnd") {
// 			storedImage = document.getElementById("storedImage_gameisover");
// 		}

		
		if (storedImage) {
			span = document.getElementById("success_or_fail_imagePlacer");
			span.innerHTML="";
			...
			newImage.innerHTML=storedImage.innerHTML;
	
			copyAttributes(storedImage, newImage);
			
			handlePotentialAudio(statusResponse.frontEndEvent, true);
		}
	}
}
...
function fillStatus(statusResponse)
{
	...
	fillScoreEditor(statusResponse);
	<% } %>
	...
	// Show a fading checkmark if the current player got a word correct.
	fillSuccessOrFailFadingImageAndPlaySound(statusResponse);
...
// 	const globalVars={};
    ...
    globalVars.currentPlayerId = null;
}
...
function init()
{
	...
	let element, rad, elements, audioElement,
		inputRange;
	...
	inputRange = document.getElementById("volumeSlider");
	inputRange.value = Math.trunc(audioElement.volume * 100);

	element = document.getElementById("radioSilly");
	element.click();
...
<div class="divTableRow">
	...
	<div class="divTableCell rightBlackBorder">
		...
		<form name="soundForm">
			...
			<input id="radioNormal" type="radio" name="soundOption"/>Normal<br/>
	...
	<div class="divTableCell">
		Activity log
		<span class="solidBlackBorder"><input type="checkbox" id="autoscrollCheckbox" checked="checked"></input> Autoscroll</span>
		<br/>
...
<tr>
	...
	<td class="mediumHeading" style="vertical-align:top">
	...
	<td class="solidBlackBorder" style="vertical-align:top">
		<span id="success_or_fail_imagePlacer"></span>
		<div style="display:flex; flex-direction:column; height:100%;">
			<%-- Current team section --%>
			<div id="upperTeamMessageDiv">
				Current player:<br/>
				<div id="currentPlayerDiv"></div>
			</div>
...
<table class="topBlackBorder bottomBlackBorder">
<tr>

<!-- Create new team section -->
<td class="rightBlackBorder" style="vertical-align: top">
	Create new team<br/>
	<input id="administration_createNewTeamText" type="text"/><br/>
	<button onclick="createNewTeam(event);">Create Team</button>
</td>

<!-- Current player changer -->
<td class="rightBlackBorder" style="vertical-align: top">
   Supplant current player<br/>
   <select id="administration_supplantCurrentPlayerSelectionList" size="1"></select><br/><br/>
   <button onclick="supplantCurrentPlayerAjax(event)">Supplant player</button>
</td>

<!-- Current player turn ender -->
<td style="vertical-align: top">
   Alternately, end the current player's turn<br/>
   <button onclick="endCurrentPlayerTurnAjax(event)">End turn</button>
</td>
...
<%-- Score editor table --%>
<div class="solidRedBorder">
	Score editor
	<button onclick="submitChangesToScore(event)">Submit changes</button><br/>
	<span class="solidBlackBorder"><input type="checkbox" id="scoreEditor_autoscrollCheckbox" checked="checked"></input> Autoscroll</span>
	<br/>
	<div class="scrollableTable solidBlackBorder" id="scoreEditor_scrollableDiv">
		<table class="scrollableTable" id="scoreEditor_scoreTable">
			<tr>
				<th class="bottomBlackBorder rightBlackBorder">id</th>
				<th class="bottomBlackBorder rightBlackBorder">Person who clicked</th>
				<th class="bottomBlackBorder rightBlackBorder">Current player at time of advancing card</th>
				<th class="bottomBlackBorder rightBlackBorder">Card text</th>
				<th class="bottomBlackBorder rightBlackBorder">Score type</th>
				<th class="bottomBlackBorder rightBlackBorder">Undo changes</th>
				<th class="bottomBlackBorder">Select for deletion</th>
			</tr>
			<tbody id="scoreEditor_scoreTableGuts"></tbody>
		</table>
	</div>
</div>
...
<%-- Various parts that start out invisible. --%>


<%-- Images for checkmark and red x. --%>
<div style="display:none">
   <img id="storedImage_success1" class="fadingImage" src="media/success_check_mark.png"></img>
   <img id="storedImage_success3" class="fadingImage" src="media/success_check_mark_x3.png"></img>
   <img id="storedImage_failure" class="fadingImage" src="media/failure_red_x.png"></img>
   
   <audio id="audioTag" src="<%= soundNormalFailureBuzz %>"></audio>
</div>
